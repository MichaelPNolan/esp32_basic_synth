...
void setArpState(float value){  
   if(value > 0.5f){
      arpState = HIGH;
      miniScreenString(2,1,"Arpeg-On!",true);
   }    else{
      arpState = LOW;
      miniScreenString(2,0,"Arpeg-Off",true);
   }
...
void setArpVariation(float value){ // enum{up,down,walk,threetwo,fourthree,randArp,entry,doubleTap};
  ...
  switch(division){
    case 1:
      arpPlayMethod = up;
      miniScreenString(3,1,"V.Upward",true);
      break;
    case 2:
      arpPlayMethod = down;
      miniScreenString(3,1,"V.Down",true);
      break;
    case 3:
      arpPlayMethod = walk;
      miniScreenString(3,1,"V.Walk",true);
      walkResume = 0;
      break;
    case 4:
      arpPlayMethod = threetwo;
      miniScreenString(3,1,"V.3>2<",true);
      break;
    case 5:
      arpPlayMethod = fourthree;
      miniScreenString(3,1,"V.4>3<",true);
      break;
    case 6:
      arpPlayMethod = randArp;
      miniScreenString(3,1,"V.Random",true);
      break;
    case 7:
      arpPlayMethod = entry;  //according to when they were entered
      miniScreenString(3,1,"V.entry",true);
      break;
    case 8:
      arpPlayMethod = doubleTap;
      miniScreenString(3,1,"V.2-tap",true);
      break;
  }
  if(heldNotes > 0)
    updatePatternOrder();
...
void setArpHold(float value){ //to be coded
   if(value > 0.5f){
      arpHold = HIGH;
      miniScreenString(4,1,"HOLD ON",HIGH);
   }else{
      arpHold = LOW;
      arpAllOff();  // this should send note off
      miniScreenString(4,0,"HOLD OFF",HIGH);
   }
}
...
void setArpNoteLength(float value){ //{ NoteQuarter,NoteEighth,NoteSixteenth,NoteQuarterTrip,NoteEighthTrip,NoteSixteenthTrip,NoteHalf,NoteHalfTrip};
  ...
  switch(division){
    case 1:
      arpNoteLength = NoteQuarter;
      miniScreenString(5,1,"Quarter",HIGH);
      break;
    case 2:
      arpNoteLength = NoteEighth;
      miniScreenString(5,1,"Eighth",HIGH);
      break;
    case 3:
      arpNoteLength = NoteSixteenth;
      miniScreenString(5,1,"Sixtnth",HIGH);
      break;
    case 4:
      arpNoteLength = NoteQuarterTrip;
      miniScreenString(5,1,"QuarTrip",HIGH);
      break;
    case 5:
      arpNoteLength = NoteEighthTrip;
      miniScreenString(5,1,"EighTrip",HIGH);
      break;
    case 6:
      arpNoteLength = NoteSixteenthTrip;
      miniScreenString(5,1,"SixtnTrip",HIGH);
      break;
    case 7:
      arpNoteLength = NoteHalf;
      miniScreenString(5,1,"NoteHalf",HIGH);
      break;
    case 8:
      arpNoteLength = NoteHalfTrip;
      miniScreenString(5,1,"HalfTrip",HIGH);
      break;
  };
}
#endif
